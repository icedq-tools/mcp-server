...
1. `list_connections` → map connection names to connectionIds. Unresolved → flag, stop.
2. `list_schemas` → confirm schema exists. Missing → flag, stop.
3. `list_tables` → confirm source + target tables exist. Missing → flag, stop.
4. `list_columns` for both tables → get live column list + icedqDatatypes.
5. Verify every mapping-doc column exists in live schema. Flag phantoms — never create rules for them.
6. `list_workspaces` + `list_folders` → resolve workspace + folder. Propose creating folder if missing; wait for
   approval.
...
1. `fetch_sample_data` — inspect values, nulls, formats
...
4. `get_guidance('groovy_expressions')` + `get_guidance('validation_rule')`
   Use profiling to confirm/adjust NotNull, ValidValues, length, format checks.

**6b Duplicate:** `profile_data` on key cols — confirm distinct=total; `get_guidance('duplicate_rule')`

**6c Checksum:** reuse profile row counts; `get_guidance('checksum_rule')`

**6d Recon:** `analyze_recon_mapping` → confirm join key + column mapping; `get_guidance('recon_rule')`
Use results to: confirm join key alignment; identify decode/type-cast cols needing custom expressions; flag SKIP cols.

**6e Pushdown:** `fetch_sample_data` on child+parent tables; `get_guidance('pushdown_rule')`

**Checklist before Rule Plan:**

- [ ] Connections resolved · Schemas confirmed · Tables confirmed · Columns confirmed
- [ ] fetch_sample_data done (src + tgt) · profile_data done · suggest_quality_checks done
- [ ] analyze_recon_mapping done (if join key exists)
- [ ] get_guidance('groovy_expressions') + get_guidance(rule_type) for each planned rule type
...
CHECKSUM: {Src}_vs_{Tgt}_RowCount_Checksum | Table mode | S.[SRC_CNT]-T.[TGT_CNT]==0
...
**Columns:** Only use columns from `list_columns`. Flag phantoms. Never propose Rule Plan until all schemas/tables/columns confirmed.
...
**Process:** Complete Step 6 before Rule Plan. Present full plan before any create_* call. Call `get_guidance` for each
rule type before creating it.
...
- Confirm with `fetch_sample_data` + `typeof()` before writing arithmetic.
