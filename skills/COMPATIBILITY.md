...
- A named REST API/endpoint → `icedq-api-testing`, whether the ask is self-validation or
  ...
  producer.
- Run-history questions split by scope, not just object type: a *specific* rule/workflow/schedule's
  own most-recent execution (pass/fail, exception detail) → `icedq-run-and-report`; a *trend or
  pattern across many runs* of a schedule ("is this firing on cadence," "which of my scheduled
  rules keep failing") → `icedq-schedule-and-monitor`. This matters mechanically, not just
  editorially: `get_scheduler_runs_history` returns one aggregate `status` per schedule *firing*
  with no per-job breakdown, and neither it nor `list_schedules` exposes the rule/workflow IDs
  attached to a schedule at all (`list_schedules` only returns a `jobCount`) — both confirmed live.
  So "which rule failed" can't be answered by paging `get_rule_workflow_run_history` per rule
  either, since there's no job list to iterate over. The confirmed-working path is the Data
  Warehouse Analytics tools against `execution_fct`, aggregated by `executable_id`/
  `execution_status` and scoped to workspace/folder (there is no schedule identifier in that
  dataset either — folder scoping is the closest available proxy for "this schedule's rules").
